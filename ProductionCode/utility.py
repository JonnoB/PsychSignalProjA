...
def market_model(stock_ts, market_ts):
    ...
    #dep_vars = np.concatenate((m_t.values.reshape(len(m_t),1), r_t.values.reshape(len(r_t),1)),axis=1)
    dep_vars = m_t.values
...
def sentiment_model(market_model, stock_ts, sentiment_ts):
    res = detrend(1,pd.DataFrame(market_model.resid)).values
    y = t_plus_n(res,1)
    prevresids = t_plus_n_minus_1(res,0)
    r_t = t_plus_n_minus_1(stock_ts, 2)
    si_t = t_plus_n_minus_1(sentiment_ts, 2)
    dep_vars = pd.DataFrame(np.concatenate((prevresids.reshape(len(prevresids),1), r_t.values.reshape(len(r_t),1), si_t.values.reshape(len(si_t),1)), axis=1))
