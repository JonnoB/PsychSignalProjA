...
```{r}
packages <- c("dplyr", "tidyr", "ggplot2", "caret", "corrplot", "xtable", "magrittr", "zoo", "igraph", "rgexf", "MCL", "Hmisc", "gridExtra")
...
x <- sapply(1:5, function(n){
  x <- MetricList[[n]] %>% group_by(PeriodID) %>% 
    summarise(TotalClusters = length(unique(ClusterID))) %>% ungroup %>%select(TotalClusters)
  }
)
